...
void HandlePlayerState(const Uint8 *state, Humanoid *player) {
    ...
    if (state[SDL_SCANCODE_LEFT]) {
        ...
        player->animation.isFacingLeft = true; // Update the direction the player is facing
    }
    if (state[SDL_SCANCODE_RIGHT]) {
        player->movement.velocity.x = MOVE_SPEED;
        player->animation.isFacingLeft = false; // Update the direction the player is facing
    }
...
bool processEvents(SDL_Window *window, Humanoid *player) {
    ...
    while (SDL_PollEvent(&event)) {
        if (!ProcessWindowEvent(&event, &window) || !ProcessKeyEvent(&event, window)) {
            return false;
        }
    }
